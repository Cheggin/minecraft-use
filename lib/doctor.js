...
function checkSidecarVenv() {
  ...
  fail('Python venv missing', 'run: minecraft-use init');
  return false;
}
...
function checkModJar() {
  ...
  fail('Fabric mod JAR not built', 'run: minecraft-use init');
  return false;
}

function runDoctor() {
  process.stdout.write(`\n${BOLD}minecraft-use doctor${RESET}\n`);
  ...
  process.stdout.write('\n');
  if (allOk) {
    process.stdout.write(`${GREEN}${BOLD}All checks passed.${RESET} Run ${BOLD}minecraft-use start${RESET} to launch.\n\n`);
  } else {
    process.stdout.write(`${RED}${BOLD}Some checks failed.${RESET} Fix the issues above, then run ${BOLD}minecraft-use init${RESET}.\n\n`);
  }
